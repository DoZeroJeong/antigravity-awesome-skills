...
- **보안(Security)**: 루트 권한 실행을 최소화하고, 민감한 키(`API_KEYS`, `SECRET_KEY` 등)가 이미지 레이어에 남지 않도록 주의하세요 (Secret Mount 활용 등).
...
- **High Performance Build**: `pip install` 대신 `uv pip install`을 사용하여 빌드 속도 획기적 단축.
