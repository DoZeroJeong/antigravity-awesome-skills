...
| **`create-new-skill`** | 새로운 에이전트 스킬을 쉽고 빠르게 생성할 수 있도록 도와주는 도구입니다. 표준화된 템플릿을 제공합니다. |
| **`django-pro`** | Django 3.x 기반의 웹 애플리케이션 개발, 아키텍처 설계, ORM 최적화 등을 지원하는 전문가 스킬입니다. |
| **`docker-expert`** | Docker 컨테이너화, Docker Compose 오케스트레이션 및 인프라 최적화를 전문으로 하는 전문가입니다. |
| **`kafka-streaming-architect`** | Apache Kafka를 활용한 이벤트 기반 아키텍처 설계, 구현 및 운영을 전문으로 하는 아키텍트입니다. |
| **`legacy-modernizer`** | 레거시 코드베이스를 안전하게 리팩토링하고, 구형 프레임워크를 마이그레이션하며, 점진적인 현대화를 수행하는 전문가입니다. |
| **`python-backend-architect`** | 견고하고 확장 가능한 파이썬 백엔드 시스템 설계 및 구현을 위한 전문가 스킬입니다. 시스템 아키텍처, 비동기 처리, 데이터 엔지니어링, 클라우드 연동을 포괄합니다. |
...
| **`verify-skill`** | 생성된 스킬이 프로젝트 표준을 준수하는지 검증하는 스킬입니다. 스킬의 구조, 필수 파일, 및 내용의 적절성을 확인합니다. |
